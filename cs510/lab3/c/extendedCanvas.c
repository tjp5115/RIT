...
{
    extendedCanvas *new = calloc( 1, sizeof(extendedCanvas) );
    if( new != NULL ) {
    ...
    extendedCanvas *E )
{
    ...
    sfConvexShape_setOutlineColor( P, E->sc->myColor );
    sfConvexShape_setOutlineThickness( P, 1 );
    for (int i=0; i < n; i++) {
	sfVector2f pt = { x[i], E->sc->height - y[i] };
        sfConvexShape_setPoint( P, i, pt );
    }

    shapeVectorPushBack( &(E->polys), P );
}
    ...
    extendedCanvas *E )
{
    sfConvexShape *P = sfConvexShape_create();
    sfConvexShape_setPointCount( P, n );
    sfConvexShape_setFillColor( P, E->sc->myColor );
    sfConvexShape_setOutlineColor( P, E->sc->myColor );
    sfConvexShape_setOutlineThickness( P, 1 );
...
void extendedCanvas_draw( sfRenderWindow *R, extendedCanvas *E )
{
    ...
    // draw as usual
    simpleCanvas_draw( R, E->sc );

    // draw your polys
    for( i = 0; i < shapeVectorSize(&(E->polys)); ++i ) {
    	sfRenderWindow_drawConvexShape( R, E->polys.vec[i], 0 );
    }
}
...
{
    simpleCanvas_setColor( r, g, b, E->sc );
}
...
void extendedCanvas_setPixelColor( int x, int y, float r, float g, float b,
    extendedCanvas *E )
{
    simpleCanvas_setPixelColor( x, y, r, g, b, E->sc );
}
...
{
    simpleCanvas_setPixel( x, y, E->sc );
}
